...
export default function ProjectEstimationPage() {
  ...
  const [estimations, setEstimations] = useState([
    { 
      id: "EST-4021", 
      project: projects[0]?.name || "Modern Villa", 
              ...
              {filtered.map((est) => (
                <tr key={est.id} className="group hover:bg-slate-50/30 transition-colors">
                  <td className="px-8 py-6">
                    <p className="text-xs font-black text-indigo-600 uppercase tracking-widest mb-1">{est.id}</p>
                    <p className="text-sm font-bold text-slate-900">{est.project}</p>
                  </td>
                  <td className="px-8 py-6">
                    <p className="text-sm font-bold text-slate-700">{est.client}</p>
                    <p className="text-xs font-medium text-slate-400">{est.date}</p>
                  </td>
                  <td className="px-8 py-6 text-center">
                    <span className="text-sm font-black text-slate-900">${est.totalAmount.toLocaleString()}</span>
                  </td>
                  <td className="px-8 py-6 text-center">
                    <span className={cn(
                      "px-3 py-1 rounded-full text-[10px] font-black border uppercase tracking-widest inline-flex items-center gap-1.5",
                      est.status === "SENT" ? "bg-indigo-50 text-indigo-700 border-indigo-100" :
                      est.status === "DRAFT" ? "bg-slate-50 text-slate-600 border-slate-200" :
                      "bg-emerald-50 text-emerald-700 border-emerald-100"
                    )}>
                      {est.status === "SENT" ? <TrendingUp className="w-3 h-3" /> : <Clock className="w-3 h-3" />}
                      {est.status}
                    </span>
                  </td>
                  <td className="px-8 py-6">
      ...
      <Modal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} title="New Project Estimation">
        <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); setIsAddModalOpen(false); }}>
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Estimated Budget</label>
              <Input placeholder="e.g. 1500000" required />
            </div>
            <div className="space-y-2">
              <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Margin (%)</label>
              <Input type="number" placeholder="15" required />
            </div>
          </div>
          <div className="space-y-2">
